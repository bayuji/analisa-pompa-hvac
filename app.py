...
# Data dummy agar kurva melengkung proporsional seperti di gambar
...
# Header Utama
st.markdown("<h2 style='color:#f8fafc; margin-bottom:20px;'>📊 Pump Performance Analysis</h2>", unsafe_allowed_html=True)

# Baris 1: 4 buah Metric Cards
col1, col2, col3, col4 = st.columns(4)
with col1:
    st.markdown(f"<div class='metric-card'><div class='metric-title'>System Flow Rate (m³/h)</div><div class='metric-value'>{q_actual}</div><div class='metric-sub'>▲ +15.3% vs Target</div></div>", unsafe_allowed_html=True)
with col2:
    st.markdown(f"<div class='metric-card'><div class='metric-title'>Total Head (m)</div><div class='metric-value'>{h_actual:.1f}</div><div class='metric-sub'>▲ +15.8% vs Target</div></div>", unsafe_allowed_html=True)
with col3:
    st.markdown(f"<div class='metric-card'><div class='metric-title'>Pump Efficiency (%)</div><div class='metric-value'>{pump_eff}%</div><div class='metric-sub' style='color:#ef4444;'>▼ -15.6% vs BEP</div></div>", unsafe_allowed_html=True)
with col4:
    st.markdown(f"<div class='metric-card'><div class='metric-title'>Power Consumption (kW)</div><div class='metric-value'>{p_cons} kW</div><div class='metric-sub' style='color:#ef4444;'>▼ -15.8% Overload</div></div>", unsafe_allowed_html=True)

st.markdown("<br>", unsafe_allowed_html=True)

# Baris 2: Grafik Utama (Kiri) & Gauge + Mini Trends (Kanan) & Insights (Paling Kiri)
...
with layout_col1:
    ...
    # Membuat Sistem Kurva Parabola (System Curve) melewati (0,0) ke arah kanan atas
    h_system = 0.0005 * (q_plot**2)
    
    # Plotly Graph (Professional Pump Curve)
    fig = go.Figure()
    
    # 1. Manufacturer's Curve
    ...
    # Garis Proyeksi Hijau Toska neon seperti gambar
...
with layout_col2:
    st.markdown("<div style='background-color:#1e293b; padding:15px; border-radius:10px; border: 1px solid #334155;'>", unsafe_allowed_html=True)
